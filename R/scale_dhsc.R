#' @title Discrete colour scales using the dhsc_cols palette
#'
#' @description \code{scale_dhsc} provides a means to easily use the \code{
#' \link{dhsc_cols}} palette in charts. It applies the palette in order as an 
#' unnamed vector.
#'
...
scale_dhsc <- function(type = "fill", ...) {

  type = match.arg(type, c("colour", "fill"))

  cols <- unname(dhsc_cols)
